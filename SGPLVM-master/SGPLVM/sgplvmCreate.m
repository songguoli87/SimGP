...
if(isfield(options,'kernelconstraints')&&~isempty(options.kernelconstraints))
    model.kernelconstraints = options.kernelconstraints;
    model.lambda = options.lambda;
    model.gamma = options.gamma;
else
  model.kernelconstraints = false;
end

if(isfield(options,'neighborconstraints')&&~isempty(options.neighborconstraints))
    model.neighborconstraints = options.neighborconstraints;
%     model.lambda1 = options.lambda1;
%     model.lambda = options.lambda2;
    model.lambda = options.lambda;
    model.gamma = options.gamma;
else
  model.neighborconstraints = false;
end
