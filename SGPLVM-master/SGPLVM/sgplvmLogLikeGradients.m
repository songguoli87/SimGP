...
% calculate gradients for kernel constraints
gkernel = cell(1,model.numModels);
if model.kernelconstraints
        for(i = 1:1:model.numModels)
          model.comp{i}.SX = model.SX;
          model.comp{i}.lambda = model.lambda;
%           model.comp{i}.SX = model.comp{i}.y;
          gkernel{i} = cell(1,2);
          [gkernel{i}{gModel_ind} gkernel{i}{gX_ind}] = kernelconstraintGradients(model.comp{i});
...
% calculate gradients for neighbor constraints
gneighbor = cell(1,model.numModels);
if model.neighborconstraints
        for(i = 1:1:model.numModels)
          model.comp{i}.SX = model.SX;
          model.comp{i}.lambda = model.lambda;
          model.comp{i}.gamma = model.gamma;
%           model.comp{i}.SX = model.comp{i}.y;
          gneighbor{i} = cell(1,2);
          gneighbor{i}{gX_ind} = neighborconstraintGradients(model.comp{i});
          g_full{i}{gX_ind} = bsxfun(@plus, g_full{i}{gX_ind}, gneighbor{i}{gX_ind});
        end
end
