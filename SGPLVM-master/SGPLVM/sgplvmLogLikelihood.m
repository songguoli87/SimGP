...
%8. kernel constraints
if model.kernelconstraints
  for(i = 1:1:model.numModels)
    model.comp{i}.SX = model.SX;
%     model.comp{i}.SX = model.comp{i}.y;
    model.comp{i}.lambda = model.lambda;
    ll = ll + kernelconstraintLogLikelihood(model.comp{i});
  end
end
if(verbose)
  ll_part(8) = ll - sum(ll_part);
end

%9. neighbor constraints
if model.neighborconstraints
%     model.comp{1}.lambda = model.lambda1;
%     model.comp{2}.lambda = model.lambda2;
  for(i = 1:1:model.numModels)
    model.comp{i}.SX = model.SX;
%     model.comp{i}.SX = model.comp{i}.y;
     model.comp{i}.lambda = model.lambda;
     model.comp{i}.gamma = model.gamma;
    ll = ll + neighborconstraintLogLikelihood(model.comp{i});
  end
end
if(verbose)
  ll_part(8) = ll - sum(ll_part);
